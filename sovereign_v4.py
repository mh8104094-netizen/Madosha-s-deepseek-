...
app = FastAPI(title="X-MIND Natural Core", version="0.6.0")
...
GENERIC = [
    ...
    "كيف يمكنني مساعد", "كيف أقدر أساعد", "ازاي اقدر اساعد", "إزاي أقدر أساعد",
    "هل هناك شيء محدد", "ترغب في استفسار", "يمكنني مساعد",
]
...
def sanitize(text: str, user_text: str) -> str:
    ...
    if not text:
        return ""
    # Tiny models often append a generic support sentence after a usable first sentence.
    chunks = [x.strip() for x in re.split(r"(?<=[.!?؟])\s+|\n+", text) if x.strip()]
    kept = [x for x in chunks if not generic(x)]
    cleaned = " ".join(kept).strip()
    if cleaned:
        text = cleaned
    # Do not allow an Arabic user turn to leak into an English-only answer.
...
def system_prompt(user_text: str, casual: bool, retry: int = 0) -> str:
    if has_arabic(user_text):
        base = "أنت X-MIND، مساعد شخصي ذكي وقريب من المستخدم. رد بالمصري الطبيعي على معنى الرسالة الحالية مباشرة."
        if casual:
            base += " الكلام العادي يكون جملة أو جملتين، بسيط وطبيعي كصاحب ذكي فاهم السياق."
        else:
            base += " لو السؤال فعلي جاوبه بوضوح وعمليًا."
        base += " لا تعرّف نفسك من نفسك ولا تسرد قدراتك."
        if retry:
            base += " اكتب صياغة جديدة أقصر وأطبيعي من المسودة السابقة."
        return base
    base = "You are X-MIND, a smart familiar personal assistant. Reply directly to the meaning of the current message."
    if casual:
        base += " Casual conversation should be one or two natural sentences, like a familiar smart companion."
    else:
        base += " For a real question, answer clearly and practically."
    base += " Do not introduce yourself or list capabilities unless asked."
    if retry:
        base += " Write a fresh, shorter, more natural version than the previous draft."
    return base


def compact_history(session: str, user_text: str) -> list[dict]:
    # Small local cortex: the most recent turn matters far more than a long transcript.
...
async def generate(user_text: str, session: str, memory_block: str = "", research_block: str = ""):
    ...
    for attempt in range(3):
        ...
        m = await core.cortex(
            msgs,
            max_tokens=56 if casual else 190,
            temperature=.84 if casual else .66,
        )
        raw = core.clean_text(m.get("content") or "", 4000)
        cleaned = sanitize(raw, user_text)
        if cleaned:
    ...
    if candidates:
        # Prefer the least repetitive / shortest usable model-generated candidate.
        ranked = sorted(
            candidates,
            key=lambda x: (generic(x), max([similarity(x, p) for p in prior] or [0]), len(x)),
        )
        return ranked[0], 2
    return ("أنا معاك." if has_arabic(user_text) else "I'm with you."), 2


@app.get("/health")
async def health():
    online, _ = await core.cortex_online()
    return {"ok": True, "version": "0.6.0", "cortex_connected": online, "model": core.CORTEX_MODEL}
...
async def status():
    ...
    return {
        ...
        "quality_gate": {"enabled": True, "anti_repetition": True, "max_attempts": 3},
...
HTML = core.HTML.replace("CONVERSATION CORE ·", "NATURAL CORE v0.6 ·").replace("xmind_session_v4", "xmind_session_v6")
